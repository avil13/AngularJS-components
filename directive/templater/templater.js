APP.directive('templater', ['$compile', '$parse', function($compile, $parse) {
    return {
        restrict: 'E',
        reqired: 'map',
        scope: '^',
        ...
        link: function(scope, iElement, iAttrs) {
            ...
            // получение строки с атрибутами из лбекта
            ...
            // создание компанента
            var to_comp = function(str, prop) {
                var res = [];
                res.push('<div class="group-content">');
                res.push('<' + str + attr(prop) + '>');
                res.push('</' + str + '>');
                res.push('</div>');
                return res.join('');
            };
            ...
            // создание заголовка
            var to_title = function(str, obj) {
                var res = [];
                res.push('<div class="group">');
                res.push('<div class="group-title">');
                res.push(str);
                res.push('</div>');
                res.push(parse(obj));
                res.push('</div>');
            ...
            var parse = function(tmpl) {
                ...
                for (k in tmpl) {
                    ...
                    switch (k.replace(reg, '$1')) {
                        case 'comp':
                            html_tmpl.push(to_comp(k.replace(reg, '$2'), val));
                            break;
                        case 'title':
                            html_tmpl.push(to_title(k.replace(reg, '$2'), val));
                            break;
                        case 'if':
